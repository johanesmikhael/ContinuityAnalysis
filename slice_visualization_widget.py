...
class SliceVisualizationWidget(qtViewer3d):
    def __init__(self, *kargs):
        super(SliceVisualizationWidget, self).__init__(*kargs)
        self._parent = kargs[0]

    def InitDriver(self):
        super(SliceVisualizationWidget, self).InitDriver()
    ...
    def mouseMoveEvent(self, event):
        super(SliceVisualizationWidget, self).mouseMoveEvent(event)
        self.update()

    def wheelEvent(self, event):
        super(SliceVisualizationWidget, self).wheelEvent(event)
        self.update()
